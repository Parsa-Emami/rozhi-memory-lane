...
<head>
    ...
    <title>{{ $title ?? 'Rozhi Coffee Plan' }}</title>
...
<div class="page-shell">
    ...
    <header class="sticky top-0 z-40 border-b border-white/35 bg-white/55 backdrop-blur-2xl dark:border-white/10 dark:bg-night/70">
        <nav class="safe-x mx-auto flex max-w-7xl items-center justify-between gap-3 py-3 sm:py-4">
            <a href="{{ route('home') }}" class="brand-mark">
                <span class="brand-icon"><img src="{{ asset(config('rozhi.cat_image')) }}" onerror="this.onerror=null;this.src='{{ asset('images/black-kitten.svg') }}'" alt="black kitten"></span>
                <span class="hidden sm:inline">Rozhi Coffee Plan</span>
            </a>

            <div class="hidden items-center gap-1 lg:flex">
                <a class="nav-link" href="{{ route('story') }}">Spin</a>
                <a class="nav-link" href="{{ route('memories') }}">Cat Deck</a>
                <a class="nav-link" href="{{ route('quiz') }}">Plan Picker</a>
                <a class="nav-link" href="{{ route('letter') }}">Note</a>
                <a class="nav-link" href="{{ route('future') }}">Ideas</a>
                <a class="nav-link nav-link-strong" href="{{ route('invite') }}">Send Invite</a>
            </div>
    ...
    <aside class="cat-companion" aria-label="black cat companion">
        <img src="{{ asset(config('rozhi.cat_image')) }}" onerror="this.onerror=null;this.src='{{ asset('images/black-kitten.svg') }}'" alt="گربه مشکی کوچک">
        <span>Pick a plan, keep it clean.</span>
    </aside>

    <div class="music-dock" aria-label="background music player">
        <audio id="bg-audio" preload="auto" loop data-src="{{ asset('audio/lana-let-the-light-in.mp3') }}"></audio>
