@extends('layouts.app', ['title' => 'Rozhi Coffee Plan'])
...
<section class="safe-x mx-auto grid max-w-7xl items-center gap-10 py-10 text-center md:grid-cols-[1.05fr_.95fr] md:py-20 md:text-right">
    <div class="animate-fadeUp">
        ...
        <div class="mt-8 flex flex-wrap justify-center gap-3 md:justify-start">
            <a href="{{ route('invite') }}" class="cute-button w-full sm:w-auto">دیدن دعوت اصلی ☕</a>
            <a href="{{ route('story') }}" class="secondary-button w-full sm:w-auto">بذار شانسی انتخاب کنه 🎛️</a>
        </div>

        <div class="mx-auto mt-8 grid max-w-xl gap-3 sm:grid-cols-3 md:mx-0">
            <a href="{{ route('future') }}" class="mini-plan-card">
                <span>☕</span>
                <strong>قهوه</strong>
    ...
    <div class="relative animate-floaty">
        <div class="hero-phone glass-card p-4">
            <div class="relative overflow-hidden rounded-[2.25rem] bg-gradient-to-br from-night via-nightSoft to-[#3a264b] p-6 text-white shadow-nightGlow">
                ...
                <img src="{{ asset(config('rozhi.cat_image')) }}" onerror="this.onerror=null;this.src='{{ asset('images/black-kitten.svg') }}'" alt="black kitten" class="relative z-10 mx-auto mt-5 w-52 drop-shadow-2xl">
                ...
                <div class="relative z-10 mt-6 rounded-[2rem] border border-white/10 bg-white/10 p-5 backdrop-blur-xl">
                    ...
                    <div class="mt-5 grid gap-3">
                        <a href="{{ route('future') }}" class="hero-list-item">☕ قهوه کوتاه</a>
                        <a href="{{ route('future') }}" class="hero-list-item">🎸 گیتار زدن</a>
                        <a href="{{ route('future') }}" class="hero-list-item">🚗 رودتریپ کوتاه</a>
                    </div>
                </div>
            </div>
        </div>
...
<section class="safe-x mx-auto max-w-7xl pb-16">
    <div class="grid gap-5 md:grid-cols-3">
        @foreach($featuredMemories as $memory)
            <article class="glass-card p-6 transition hover:-translate-y-2">
                <p class="text-4xl">{{ $memory->mood === 'coffee' ? '☕' : ($memory->mood === 'guitar' ? '🎸' : '🚗') }}</p>
                <h2 class="mt-4 text-2xl font-black">{{ $memory->title }}</h2>
                <p class="mt-2 text-sm font-black text-blush-400">{{ $memory->place }}</p>
                <p class="mt-4 leading-8 text-cocoa/70 dark:text-white/70">{{ $memory->description }}</p>
            </article>
        @endforeach
    </div>
</section>
